...
root_directory = _detect_root_directory()
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-warnings",
        action="store_true",
        help="If passed, generation succeeds even when XML warnings are reported.",
    )
    ...
    topic_docs = discover_topic_docs()
    ...
    print("")
    if topic_docs:
        print("Generating the guide pages...")
        make_rst_topic_docs(topic_docs, args.dry_run, args.output)
        print("")

    print("Generating the index file...")

    make_rst_index(grouped_classes, topic_docs, args.dry_run, args.output)
    ...
    success = state.num_errors == 0 and (state.num_warnings == 0 or args.allow_warnings)
    ...
    if state.num_warnings == 0 and state.num_errors == 0:
        ...
        print(f"{Ansi.YELLOW}Warnings were ignored due to --allow-warnings.{Ansi.RESET}")
    else:
        exit(1)
...
def discover_topic_docs() -> List[TopicDoc]:
    root = topic_docs_root()
        ...
        for file_name in sorted(file_names):
            ...
            topic_docs.append(
                TopicDoc(
                    ...
                    slug=sanitize_topic_doc_name(relative_path),
                    title=extract_markdown_title(source_path),
                    top_level=file_name.endswith("Guide.md"),
                )
            )

    topic_docs.sort(key=lambda doc: doc.source_path)
    return topic_docs
...
def convert_markdown_inline(text: str, current_path: str, current_slug: str, slug_by_path: Dict[str, str]) -> str:
    code_spans: List[str] = []
    ...
    def replace_link(match: re.Match[str]) -> str:
        ...
        if url.startswith("#"):
            anchor = markdown_anchor(url[1:] or title)
            return f"`{title} <#{current_slug}__{anchor}>`__"

        if url.startswith("http://") or url.startswith("https://"):
            return f"`{title} <{url}>`__"

        if url.endswith(".md"):
            resolved = os.path.abspath(os.path.normpath(os.path.join(os.path.dirname(current_path), url)))
            slug = slug_by_path.get(resolved)
            if slug:
                return f":doc:`{title} <{slug}>`"
...
def render_markdown_as_rst(doc: TopicDoc, slug_by_path: Dict[str, str]) -> str:
    out: List[str] = [
        ":github_url: hide",
        ":allow_comments: False",
        "",
        ".. DO NOT EDIT THIS FILE!!!",
        f".. Generated automatically from {os.path.relpath(doc.source_path, topic_docs_root())}.",
        "",
    ]
    ...
    for line in lines:
        ...
        if stripped.startswith("```"):
            if in_code_block:
                ...
                out.append(f".. code-block:: {fence_language or 'text'}")
                out.append("")
                in_code_block = True
            continue
        ...
        if heading_match:
            level = min(len(heading_match.group(1)), len(HEADING_MARKERS))
            title = convert_markdown_inline(heading_match.group(2).strip(), doc.source_path, doc.slug, slug_by_path)
            anchor = markdown_anchor(heading_match.group(2))
            if anchor:
                out.append(f".. _{doc.slug}__{anchor}:")
                out.append("")
            out.append(title)
            out.append(HEADING_MARKERS[level - 1] * max(len(title), 1))
            out.append("")
            continue
        ...
        if bullet_match:
            ...
            bullet_text = convert_markdown_inline(bullet_match.group(2), doc.source_path, doc.slug, slug_by_path)
        ...
        if number_match:
            ...
            item_text = convert_markdown_inline(number_match.group(3), doc.source_path, doc.slug, slug_by_path)
            out.append(f"{indent}{number}. {item_text}")
            continue

        out.append(convert_markdown_inline(line, doc.source_path, doc.slug, slug_by_path))
...
def make_rst_topic_docs(topic_docs: List[TopicDoc], dry_run: bool, output_dir: str) -> None:
    slug_by_path = {doc.source_path: doc.slug for doc in topic_docs}

    for doc in topic_docs:
        target_path = os.devnull if dry_run else os.path.join(output_dir, f"{doc.slug}.rst")
        with open(target_path, "w", encoding="utf-8", newline="\n") as f:
            f.write(render_markdown_as_rst(doc, slug_by_path))


def make_rst_index(grouped_classes: Dict[str, List[str]], topic_docs: List[TopicDoc], dry_run: bool, output_dir: str) -> None:
    with open(
        ...
        guide_docs = [doc for doc in topic_docs if doc.top_level]
        supporting_docs = [doc for doc in topic_docs if not doc.top_level]
        if guide_docs:
            f.write(make_heading("Guides", "="))
            f.write(".. toctree::\n")
            f.write("    :maxdepth: 1\n")
            f.write("    :name: toc-guides\n")
            f.write("\n")
                    ...
                    if (
                        ...
                        if target_class_name in state.classes:
                            ...
                            if tag_state.name == "method":
                                ...
                                if link_target.find(".") == -1 and "@GlobalScope" in state.classes:
